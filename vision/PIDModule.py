...
class PID:
    ...
    def draw(self, img, cVal):
        h, w, _ = img.shape
        if self.axis == 0:
            cv2.line(img, (self.targetVal, 0),
                     (self.targetVal, h), (255, 0, 255), 1)
            cv2.line(img, (self.targetVal, cVal[1]),
                     (cVal[0], cVal[1]), (255, 0, 255), 1, 0)
        else:
            cv2.line(img, (0, self.targetVal),
                     (w, self.targetVal), (255, 0, 255), 1)
            cv2.line(img, (cVal[0], self.targetVal),
                     (cVal[0], cVal[1]), (255, 0, 255), 1, 0)
